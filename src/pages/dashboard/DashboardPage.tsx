...
import { TrendingUp, FileText, AlertTriangle, Building2, Award, ChevronRight } from 'lucide-react'
...
// Helper para extraer data de allSettled sin explotar si falla una query
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function ok(result: PromiseSettledResult<any>): any[] {
  if (result.status === 'rejected') { console.warn('Dashboard query rejected:', result.reason); return [] }
  if (result.value?.error) { console.warn('Dashboard query error:', result.value.error); return [] }
  return result.value?.data ?? []
}

const STAGE_LABEL: Record<string, string> = {
  nuevo: 'Nuevo', en_negociacion: 'Negociación', propuesta_enviada: 'Propuesta',
...
// ─── primitives ───────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, icon: Icon, color = 'text-gray-500', alert = false }: {
  ...
  return (
    <div className={`bg-white rounded-xl border p-5 flex items-start gap-4 ${isAlert ? 'border-red-200 bg-red-50' : ''}`}>
...
function KamDashboard({ profile }: { profile: Profile }) {
  ...
  async function load() {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        supabase.from('deals').select('id, stage, expected_value, currency')
          .eq('kam_id', id).not('stage', 'in', '(ganado,perdido)'),
        supabase.from('deals').select('id, expected_value')
        ...
        supabase.from('quotes').select('id, quote_number, total, currency, valid_until, company:companies(name)')
          .eq('kam_id', id).eq('status', 'enviada').order('created_at', { ascending: false }),
        supabase.from('deals').select('id, title, expected_close, expected_value, currency, company:companies(name)')
          .eq('kam_id', id).not('stage', 'in', '(ganado,perdido)')
          .gte('expected_close', t).lte('expected_close', in30).order('expected_close', { ascending: true }).limit(5),
      ...
      const [activeDeals, wonDeals, overdueActs, upcomingActs,
      ...
      setData({
        totalPipeline:    (activeDeals as any[]).reduce((s, d) => s + (d.expected_value ?? 0), 0),
        activeCount:      (activeDeals as any[]).length,
        byStage,
        totalWon:         (wonDeals as any[]).reduce((s, d) => s + (d.expected_value ?? 0), 0),
        overdueActs,
        upcomingActs,
        sentQuotes,
        closingSoon,
  ...
  if (loadError) return (
    <div className="text-red-400 text-sm py-16 text-center">
      Error al cargar el dashboard. Revisa la consola del navegador.
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Pipeline activo"            value={fmtCLP(totalPipeline)} sub={`${activeCount} negocios`} icon={TrendingUp} color="text-blue-500" />
        ...
        <StatCard label="Cotiz. esperando respuesta" value={(sentQuotes as any[]).length}   icon={FileText}      color="text-violet-500" />
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2">
          <Section title="Pipeline por etapa" action={<SeeAll to="/deals" />}>
            <div className="p-5 space-y-3.5">
              {ACTIVE_STAGES.map(stage => {
                const s = byStage[stage] ?? { count: 0, value: 0 }
                return (
                  <div key={stage} className="cursor-pointer group" onClick={() => navigate('/deals')}>
              ...
              {activeCount === 0 && <Empty msg="Sin negocios activos" />}
            </div>
          </Section>
        </div>
        ...
        <Section title={`Seguimientos vencidos (${(overdueFollowups as any[]).length})`}>
          {(overdueFollowups as any[]).length === 0
            ...
            : <ul className="divide-y">
                {(overdueFollowups as any[]).map((c) => (
                  <li key={c.id} onClick={() => navigate('/calls')}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Section title="Cierres próximos — 30 días" action={<SeeAll to="/deals" />}>
          {(closingSoon as any[]).length === 0 ? <Empty msg="Sin cierres próximos" /> : (
            <ul className="divide-y">
              {(closingSoon as any[]).map((d) => (
                <li key={d.id} onClick={() => navigate('/deals')}
                  ...
                  <div className="text-right shrink-0">
                    {d.expected_value != null && <p className="text-sm font-semibold text-gray-800">{fmtCLP(d.expected_value)}</p>}
        ...
        <Section title={`Tareas pendientes${(overdueActs as any[]).length > 0 ? ` · ${(overdueActs as any[]).length} vencidas` : ''}`}>
          {(overdueActs as any[]).length === 0 && (upcomingActs as any[]).length === 0
            ? <Empty msg="Sin tareas pendientes" />
            : <ul className="divide-y">
                {(overdueActs as any[]).map((a) => (
                  <li key={a.id} className="px-5 py-2.5 flex items-center gap-3">
                    <span className="text-[10px] bg-red-100 text-red-600 rounded px-1.5 py-0.5 shrink-0 font-medium">VENCIDA</span>
                ...
                {(upcomingActs as any[]).map((a) => (
                  <li key={a.id} className="px-5 py-2.5 flex items-center gap-3">
                    <span className="text-[10px] bg-gray-100 text-gray-500 rounded px-1.5 py-0.5 shrink-0">{a.type}</span>
                    <p className="text-xs font-medium text-gray-900 flex-1 truncate">{a.title}</p>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Section title="Cotizaciones esperando respuesta" action={<SeeAll to="/quotes" />}>
          {(sentQuotes as any[]).length === 0 ? <Empty msg="Sin cotizaciones pendientes" /> : (
            <ul className="divide-y">
              {(sentQuotes as any[]).slice(0, 6).map((q) => (
                <li key={q.id}
                  onClick={() => navigate('/quotes', { state: { highlightId: q.id } })}
                  className="px-5 py-3 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition-colors">
                  <div className="min-w-0 flex-1">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-gray-800">{fmtTotal(q.total, q.currency)}</p>
                    {q.valid_until && (
...
function AdminDashboard({ profile }: { profile: Profile }) {
  ...
  async function load() {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        supabase.from('deals').select('id, stage, expected_value, kam_id, kam:profiles(full_name)')
          .not('stage', 'in', '(ganado,perdido)'),
        supabase.from('deals').select('id, expected_value, kam_id, kam:profiles(full_name)')
          .eq('stage', 'ganado').gte('closed_at', som),
        supabase.from('quotes').select('id, status'),
        supabase.from('company_kams').select('company_id'),
        supabase.from('deals').select('id, title, stage, updated_at, company:companies(name), kam:profiles(full_name)')
          .not('stage', 'in', '(ganado,perdido)').lt('updated_at', ago30)
        ...
        supabase.from('quotes').select('id, quote_number, valid_until, total, currency, company:companies(name), kam:profiles(full_name)')
          .eq('status', 'enviada').not('valid_until', 'is', null)
          .lt('valid_until', t).order('valid_until', { ascending: true }).limit(6),
      ])

      const [companies, allDeals, wonThisMonth, allQuotes,
             companyKams, staleDeals, recentCalls, expiredQuotes] = results.map(ok)
      ...
      for (const d of allDeals as any[]) {
        const key = d.kam_id ?? '__none__'
        if (!pipelineByKam[key]) pipelineByKam[key] = { name: d.kam?.full_name ?? 'Sin KAM', value: 0, count: 0 }
        pipelineByKam[key].value += d.expected_value ?? 0
        pipelineByKam[key].count++
      }
      const wonByKam: Record<string, { name: string; value: number; count: number }> = {}
      for (const d of wonThisMonth as any[]) {
        const key = d.kam_id ?? '__none__'
        if (!wonByKam[key]) wonByKam[key] = { name: d.kam?.full_name ?? 'Sin KAM', value: 0, count: 0 }
        wonByKam[key].value += d.expected_value ?? 0
        wonByKam[key].count++
      }
      ...
      setData({
        ...
        totalPipeline:  (allDeals as any[]).reduce((s, d) => s + (d.expected_value ?? 0), 0),
        openDeals:      (allDeals as any[]).length,
        totalWon:       (wonThisMonth as any[]).reduce((s, d) => s + (d.expected_value ?? 0), 0),
        sentCount:      (allQuotes as any[]).filter(q => (q as any).status === 'enviada').length,
  ...
  const { activeCount, withoutKamCount, totalPipeline, openDeals, totalWon,
    sentCount, pipelineByKam, wonByKam, staleDeals, recentCalls, expiredQuotes } = data
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Section title="Pipeline activo por KAM" action={<SeeAll to="/deals" />}>
          <div className="p-5 space-y-3.5">
            {(pipelineByKam as any[]).length === 0 ? <Empty msg="Sin negocios activos" /> :
              (pipelineByKam as any[]).map(k => (
                <div key={k.name} className="cursor-pointer group" onClick={() => navigate('/deals')}>
                  <div className="flex justify-between text-xs mb-1.5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Section title={`Negocios sin actividad +30 días (${(staleDeals as any[]).length})`} action={<SeeAll to="/deals" />}>
          {(staleDeals as any[]).length === 0 ? <Empty msg="Sin negocios estancados ✓" /> : (
            <ul className="divide-y">
              {(staleDeals as any[]).map(d => (
                <li key={d.id} onClick={() => navigate('/deals')}
                  className="px-5 py-2.5 flex items-center gap-3 cursor-pointer hover:bg-gray-50 transition-colors">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 truncate">{d.title}</p>
                    <p className="text-xs text-gray-400 truncate">{d.company?.name} · {d.kam?.full_name}</p>
                  </div>
          ...
          <div className="grid grid-cols-2 divide-x">
            {[(recentCalls as any[]).slice(0, 4), (recentCalls as any[]).slice(4, 8)].map((half, col) => (
              <ul key={col} className="divide-y">
                {half.map((c: any) => (
                  <li key={c.id} onClick={() => navigate('/calls')}
                    className="px-5 py-3 flex items-center gap-2 cursor-pointer hover:bg-gray-50 transition-colors">
